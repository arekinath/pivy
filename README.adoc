...
`/opt/pivy`.  The `make setup` invocation can be used to set up a user
...
$ sudo make install
install -o root -g wheel -m 0755 -d /opt/pivy/bin
install -o root -g wheel -m 0755 pivy-agent /opt/pivy/bin
install -o root -g wheel -m 0755 pivy-tool /opt/pivy/bin
